...
def get_color_ordered_labels(stats_by_label: Dict[str, any]) -> List[str]:
    """
    Return labels in the desired color order.
    Based on the launch config: "GRPO (A), CAPO, DrGRPO, DrCAPO, REINFORCE, ReinCAPO"
    """
    # Debug: print actual labels
    logger.info(f"Available labels: {list(stats_by_label.keys())}")
    
    # Define the desired order (matching the actual labels from launch config)
    desired_order = ["DrCAPO", "GRPO (A)", "CAPO", "DrGRPO", "REINFORCE", "ReinCAPO"]
    
    # Plot in the exact order specified
    ordered_labels = []
    used_labels = set()
    
    for desired_label in desired_order:
        if desired_label in stats_by_label:
            ordered_labels.append(desired_label)
            used_labels.add(desired_label)
            logger.info(f"Added '{desired_label}' to ordered list")
    
    # Add any remaining labels that weren't in the desired order
    for label in stats_by_label.keys():
        if label not in used_labels:
            ordered_labels.append(label)
            logger.info(f"Added unmatched label: '{label}'")
    
    logger.info(f"Final ordered labels: {ordered_labels}")
...
def plot_train_metric_subplot(ax, run_paths: List[str], x_metric: str,
    ...
    # Get labels in the desired color order
    ordered_labels = get_color_ordered_labels(stats_by_label)
...
def plot_eval(run_paths: List[str], x_metric: str, avg_group: List[str], output_dir: str,
    ...
    out = Path(output_dir) / "capo_extensions.png"
    out.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out, dpi=200)
    
    out_pdf = Path(output_dir) / "capo_extensions.pdf"
